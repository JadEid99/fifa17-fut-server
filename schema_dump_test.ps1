# Schema Dump Test — dumps the PreAuthResponse TDF member info table
#
...
#   3. Attach Frida with schema dump script (one-shot, exits immediately)
...
# Attach Frida with schema dump (one-shot — script runs and exits)
...
# The schema dump script is one-shot (no hooks, just reads memory).
...
git add -A | Out-Null
git commit -m "schema dump $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Out-Null
