# Frida Login Flow Trace - Automated
# Launches game, attaches Frida, triggers connection, collects results
...
"@
function Focus { $p=Get-Process -Name FIFA17 -EA SilentlyContinue; if($p -and $p.MainWindowHandle -ne [IntPtr]::Zero){[KF]::SetForegroundWindow($p.MainWindowHandle)|Out-Null;Start-Sleep -Milliseconds 300;return $true};return $false }
function FEnter { if(Focus){[KF]::Enter()} }
function FQ { if(Focus){[KF]::Q()} }

Write-Host "=== Frida Login Flow Trace ===" -ForegroundColor Cyan
...
$bs1 = if($blazeOut.Length -gt 2000){$blazeOut.Substring($blazeOut.Length-2000)}else{$blazeOut}
$dl1 = if($dllLog.Length -gt 2000){$dllLog.Substring($dllLog.Length-2000)}else{$dllLog}
$fr1 = if($fridaOut.Length -gt 3000){$fridaOut.Substring($fridaOut.Length-3000)}else{$fridaOut}

$results = @"
=== Frida Login Trace ($(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')) ===

--- FRIDA OUTPUT (last 3000 chars) ---
$fr1

--- BLAZE SERVER LOG (last 2000 chars) ---
$bs1

--- DLL LOG (last 2000 chars) ---
$dl1
"@
Set-Content $resultsFile $results -Encoding UTF8

# Push
git add -A; git commit -m "Frida trace results $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"; git push 2>&1
